...
qu_calculation <- function(s_ik = s_ik, t_jg = t_jg, x = x, mu_i = mu_i, nu_j = nu_j,
    alpha_c = alpha_c, a = a) {
    ...
    return(list(a_tilde = a_tilde, b_tilde = b_tilde, exp_utilde = exp_utilde, exp_logutilde = exp_logutilde))
}
...
qukg_calculation <- function(s_ik = s_ik, t_jg = t_jg, x = x, mu_i = mu_i, nu_j = nu_j,
    alpha_c = alpha_c, a = a) {
...
lb_calculation <- function(x = x, qu_param = qu_param, s_ik = s_ik, pi_c = pi_c,
    t_jg = t_jg, rho_c = rho_c, mu_i = mu_i, nu_j = nu_j, alpha_c = alpha_c, a = a,
    akg = TRUE) {
...
penalty <- function(x) {
    ...
    if (x$strategy$akg == TRUE) {
        penalty <- 0.5 * (K - 1) * log(n) + 0.5 * (G - 1) * log(m) + 0.5 * (2 * K *
            G) * log(m * n)
    } else {
        penalty <- 0.5 * (K - 1) * log(n) + 0.5 * (G - 1) * log(m) + 0.5 * (K * G) *
            log(m * n)
    }
    return(penalty)
}
